...
public class TemperatureFilter extends FilterFramework
{
	public void run()
    {
		...
		double temperature_farenheit = 0.0;	// This is the double to store the temperature in farenheit
		double temperature_celsius = 0.0; 	// This is the double to store the temperature in celsius
...
***************************************************************/
			...
			try
			{
				...
				/****************************************************************************
				 * Here we pick up the temperature measurement (ID = 4).
				 *****************************************************************************/
				if ( id == 4 )
				{
					// We convert the bytes to a double and store them in the temperature_farenheit variable
					temperature_farenheit = Double.longBitsToDouble(measurement);
					// We then convert the temperature from celsius to farenheit and store it in the temperature_celsius variable
					temperature_celsius = (temperature_farenheit - 32);
					temperature_celsius = (temperature_celsius * 5)/9;
				} // if
				...
				// If it's the temperature ID, the measurement is a double
				if(id==4) {
					bytes = ByteBuffer.allocate(MeasurementLength).putDouble(temperature_celsius).array();
				} // if
